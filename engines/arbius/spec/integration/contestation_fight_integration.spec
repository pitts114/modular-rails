...
RSpec.describe 'Contestation Fight Integration', type: :integration do
  ...
  it 'runs a full contestation fight flow with multiple wallets' do
    ...
    ethereum_transaction_id = Arbius::SubmitTaskService.new.submit_task(from: wallet1, version: 0, owner: wallet1, model:, fee:, input:)
    ...
    commitment = Arbius::GenerateCommitmentService.new.generate_commitment_onchain(sender: wallet1, taskid: task_id, cid:)
    block_number = Ethereum::LatestBlockNumberService.new.call
    ethereum_transaction_id = Arbius::SignalCommitmentService.new.call(from: wallet1, commitment: commitment)
    perform_all_resque_jobs
    ...
    block_number = Ethereum::LatestBlockNumberService.new.call
    ethereum_transaction_id = Arbius::SubmitSolutionService.new.call(from: wallet1, cid:, taskid: task_id)
    perform_all_resque_jobs
    ...
    block_number = Ethereum::LatestBlockNumberService.new.call
    ethereum_transaction_id = Arbius::VoteOnContestationService.new.call(from: wallet6, task_id: task_id, yea: true)
    ...
    Ethereum::TestEventPollService.new(default_start_block: block_number).call do
      begin
        perform_all_resque_jobs
      rescue Arbius::DefendSolutionService::OutnumberedError
        perform_all_resque_jobs
      end
      result = Ethereum::Public::TransactionStatusService.new.call(ethereum_transaction_id: ethereum_transaction_id)
      if result[:status] == 'confirmed'
