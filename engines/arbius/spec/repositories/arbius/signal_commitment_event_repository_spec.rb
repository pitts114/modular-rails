...
RSpec.describe Arbius::SignalCommitmentEventRepository do
  let(:attributes) do
    {
  ...
  describe '.save!' do
    it 'creates both an EthereumEventDetail and a SignalCommitmentEvent' do
      expect {
        described_class.save!(attributes: attributes)
      }.to change(Arbius::EthereumEventDetail, :count).by(1)
       .and change(Arbius::SignalCommitmentEvent, :count).by(1)
    end
