...
RSpec.describe Arbius::SolutionClaimedEventRepository do
  let(:attributes) do
    {
  ...
  describe '.save!' do
    it 'creates both an EthereumEventDetail and a SolutionClaimedEvent' do
      expect {
        described_class.save!(attributes: attributes)
      }.to change(Arbius::EthereumEventDetail, :count).by(1)
       .and change(Arbius::SolutionClaimedEvent, :count).by(1)
    end
