...
RSpec.describe Arbius::ContestationSubmittedEventRepository do
  let(:attributes) do
    {
  ...
  describe '.save!' do
    it 'creates both an EthereumEventDetail and a ContestationSubmittedEvent' do
      expect {
        described_class.save!(attributes: attributes)
      }.to change(Arbius::EthereumEventDetail, :count).by(1)
       .and change(Arbius::ContestationSubmittedEvent, :count).by(1)
    end
